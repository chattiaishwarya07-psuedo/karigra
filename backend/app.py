...
from .services.catalogue_service import generateCatalogueService, translateProductDescriptionService, autoFillProductDetailsService
...
# Enable CORS headers manually for maximum reliability
@app.after_request
...
def handle_artisans():
    if request.method == 'GET':
        ...
        return jsonify({"success": True, "artisan": new_artisan}), 201
...
def handle_products():
    if request.method == 'GET':
        ...
        products = ProductModel.get_all(artisan_id=artisan_id, status=status, category=category)
...
def enhance_image():
    # Handle direct file upload or image URL / preset
    uploaded_file = request.files.get('image')
    image_url = request.form.get('image_url') or (request.json.get('image_url') if request.is_json else None)
    options_raw = request.form.get('options') or (request.json.get('options') if request.is_json else None)
    ...
    if uploaded_file and uploaded_file.filename:
        filename = f"upload_{uuid.uuid4().hex[:8]}_{secure_filename(uploaded_file.filename)}"
        filepath = os.path.join(UPLOADS_DIR, filename)
        uploaded_file.save(filepath)
        orig_url = f"/uploads/{filename}"
    elif image_url:
        # Check if local asset or uploaded image
        clean_path = image_url.lstrip('/')
        if clean_path.startswith('assets/'):
            filepath = os.path.join(FRONTEND_DIR, clean_path)
        elif clean_path.startswith('uploads/'):
            filepath = os.path.join(UPLOADS_DIR, os.path.basename(clean_path))
        else:
            filepath = os.path.join(UPLOADS_DIR, os.path.basename(clean_path))
        orig_url = image_url
    else:
        # Default to raw pottery snap asset for demo
        filepath = os.path.join(FRONTEND_DIR, 'assets', 'raw_pottery_snap.jpg')
...
def process_speech():
    ...
    result = speechToTextService(language_code=lang, sample_preset_id=preset_id)
...
def generate_catalogue():
    ...
    result = generateCatalogueService(transcript=transcript, language_code=lang, craft_category=category)
    return jsonify(result)
...
def translate_description():
    ...
    category = data.get('craft_category')
    result = translateProductDescriptionService(text, source_lang, target_lang, category)
    return jsonify(result)
...
def auto_fill_details():
    ...
    category = data.get('craft_category')
    result = autoFillProductDetailsService(description, image_url=image_url, craft_category=category)
    return jsonify(result)
...
def calculate_price():
    ...
    if not isinstance(costs, dict):
        costs = {}
    if 'raw_material_cost' in data and 'raw_material_cost' not in costs:
        costs['raw_material_cost'] = data.get('raw_material_cost')
    if 'labour_hours' in data and 'labour_hours' not in costs:
        costs['labour_hours'] = data.get('labour_hours')
    if 'labour_rate_per_hour' in data and 'labour_rate_per_hour' not in costs:
        costs['labour_rate_per_hour'] = data.get('labour_rate_per_hour')

    category = data.get('craft_category') or data.get('category') or 'Handloom'
    gi_tag = data.get('gi_tag_certified', True)
    experience_years = data.get('experience_years', 10)
    result = calculatePriceRecommendation(costs, category, gi_tag, experience_years)
    return jsonify(result)
...
def create_enquiry():
    ...
    return jsonify({"success": True, "enquiry": enquiry}), 201
...
def get_market_insights():
    artisan_id = request.args.get('artisan_id', type=int)
    category = request.args.get('category')
    insights = generateMarketInsightsService(artisan_id=artisan_id, craft_category=category)
